...
def evaluate(expression, operands):
    result = operands[0]
    i, ii = 0, 0
    while True:
        if expression[ii] == '+':
            ...
            i, ii = i+1, ii+1
        ii += 1
        if ii >= len(expression) or i >= len(operands):
            break 
    return result
...
def findOperation(operands, result, operations = operations):
    ...
    doOperation(operands=operands, operations=operations,  
                curr_expression=f"{operands[0]}", i=0,
                expressions=expressions)
    
    results = []
    for expression in expressions:
        # _result = eval(expression)
        _result = evaluate(expression, operands) # <--- custom evaluator 
        if _result == result:
